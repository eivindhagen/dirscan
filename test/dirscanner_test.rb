...
require File.expand_path('../lib/indexfile_job.rb', File.dirname(__FILE__))

class TestDirScanJob < Test::Unit::TestCase
  ...
  def test_onefile_full
    ...
    ds = IndexFileJob.new(inputs, outputs)
    ...
    inputs = {files: {scan_index: 'tmp/one_file.dirscan'}}
    outputs = {}
    dv = DirScanJob.new(inputs, outputs)
  ...
  def test_onefile_quick
    ...
    assert_equal({:dir_sizes_sorted_by_count=>[[3, 1]], :file_sizes_sorted_by_count=>[[3, 1]]}, analysis_report)
  ...
  def test_dupes_quick
    ...
    analysis_report = ds.analysis_report
    assert_equal({
      :dir_sizes_sorted_by_count=>[[62, 3], [5, 2], [0, 1], [196, 1]],
      :file_sizes_sorted_by_count=>[[0, 4], [12, 3], [50, 3], [5, 2]]
    }, analysis_report)
